...
import { buildOpportunity } from "@/lib/core/build-opportunity";
import {
...
async function runScan(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const observedUniverse = runtimeConfig.BCA_V55_SHADOW_ENABLED
      ? await client.getUniverseSnapshot()
      : null;
    const universe = observedUniverse?.eligibleSymbols ?? await client.getUniverse();
    ...
    if (runtimeConfig.BCA_V55_SHADOW_ENABLED) {
      ...
      } else if (!observedUniverse) {
        await recordV55Warning(supabase, "V5.5 Shadow universe capture was unavailable; no evidence was written.", {
          scanRunId,
