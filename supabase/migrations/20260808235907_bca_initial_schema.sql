-- Binance Crypto Alerts isolated schema objects.
-- This migration intentionally creates only bca_* objects in public.

create extension if not exists pgcrypto with schema extensions;

create table public.bca_strategy_versions (
...
create table public.bca_instruments (
...
create table public.bca_scan_runs (
  id uuid primary key default gen_random_uuid(),
...
create table public.bca_signals (
  id uuid primary key default gen_random_uuid(),
  scan_run_id uuid references public.bca_scan_runs(id) on delete set null,
  ...
  replaced_by uuid references public.bca_signals(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table public.bca_signal_events (
  id uuid primary key default gen_random_uuid(),
  signal_id uuid not null references public.bca_signals(id) on delete cascade,
...
create table public.bca_risk_budgets (
...
create table public.bca_notifications (
  id uuid primary key default gen_random_uuid(),
  signal_id uuid references public.bca_signals(id) on delete set null,
...
create table public.bca_backtest_runs (
  id uuid primary key default gen_random_uuid(),
  strategy_version text not null,
...
create table public.bca_system_events (
  id uuid primary key default gen_random_uuid(),
...
create table public.bca_app_settings (
...
create unique index bca_signals_one_active_per_symbol_idx
  on public.bca_signals (symbol)
  where status = 'ACTIVE';

create index bca_signals_created_at_idx on public.bca_signals (created_at desc);
create index bca_signals_occurrence_date_idx on public.bca_signals (occurrence_date desc, score desc);
create index bca_signals_status_idx on public.bca_signals (status, symbol);
create index bca_signal_events_signal_id_idx on public.bca_signal_events (signal_id, event_at desc);
create index bca_notifications_status_idx on public.bca_notifications (status, created_at desc);
create index bca_system_events_occurred_at_idx on public.bca_system_events (occurred_at desc);
...
create trigger bca_instruments_set_updated_at
before update on public.bca_instruments
for each row execute function public.bca_set_updated_at();

create trigger bca_signals_set_updated_at
before update on public.bca_signals
for each row execute function public.bca_set_updated_at();

create trigger bca_notifications_set_updated_at
before update on public.bca_notifications
for each row execute function public.bca_set_updated_at();

create trigger bca_risk_budgets_set_updated_at
before update on public.bca_risk_budgets
for each row execute function public.bca_set_updated_at();

create trigger bca_app_settings_set_updated_at
before update on public.bca_app_settings
for each row execute function public.bca_set_updated_at();
...
insert into public.bca_app_settings (setting_key, setting_value, description)
values
