...
export interface ForwardHorizonMetrics {
...
export interface ForwardEdgeMetrics {
  ...
  forwardReturn72h: number | null;
  /** Backwards-compatible aliases for the configured max-hold horizon. */
  maxFavorableR: number;
  maxAdverseR: number;
  timeToMFEHours: number | null;
  timeToMAEHours: number | null;
  ...
  pPositiveOneRBeforeStop: boolean;
  pPositiveTwoRBeforeStop: boolean;
  horizon24h: ForwardHorizonMetrics;
  horizon72h: ForwardHorizonMetrics;
}

const DEFAULT_HORIZONS_HOURS = [0.25, 1, 4, 12, 24, 72];
...
export function calculateForwardEdge(
  ...
  const boundedMaxHoldHours = Math.max(0, maxHoldHours);
  const returns = horizonsHours.map((hours) => {
    if (hours > boundedMaxHoldHours) return null;
    const target = entryTime + hours * 3_600_000;
    const candle = candles.find((item) => item.closeTime >= target && item.closeTime <= entryTime + boundedMaxHoldHours * 3_600_000);
    return candle ? ((candle.close - entryPrice) * direction) / entryPrice : null;
  });
  const horizon24h = calculateHorizon(candles, entryIndex, side, entryPrice, riskDistance, Math.min(24, boundedMaxHoldHours));
  const horizon72h = calculateHorizon(candles, entryIndex, side, entryPrice, riskDistance, Math.min(72, boundedMaxHoldHours));
  return {
    ...
    forwardReturn72h: returns[5] ?? null,
    maxFavorableR: horizon72h.maxFavorableR,
    maxAdverseR: horizon72h.maxAdverseR,
    timeToMFEHours: horizon72h.timeToMFEHours,
    timeToMAEHours: horizon72h.timeToMAEHours,
    pPositiveHalfRBeforeStop: horizon72h.pPositiveHalfRBeforeStop,
    pPositiveOneRBeforeStop: horizon72h.pPositiveOneRBeforeStop,
    pPositiveTwoRBeforeStop: horizon72h.pPositiveTwoRBeforeStop,
    horizon24h,
    horizon72h,
  };
}
...
function calculateHorizon(
  ...
  riskDistance: number,
  horizonHours: number,
): ForwardHorizonMetrics {
  const direction = side === "LONG" ? 1 : -1;
  const entryTime = candles[entryIndex]?.closeTime ?? 0;
  const deadline = entryTime + horizonHours * 3_600_000;
  let maxFavorableR = 0;
  let maxAdverseR = 0;
  ...
  if (riskDistance > 0) {
    for (const candle of candles.slice(entryIndex + 1)) {
      ...
      if (!stopSeen) {
        // OHLC cannot reveal the intrabar path; keep the engine's stop-first convention.
        if (adverseR >= 1) {
          stopSeen = true;
        } else {
