...
export function evaluateUniverseQuality(
  ...
  candles: Candle[],
  now = Date.now(),
  policy: UniversePolicy = DEFAULT_UNIVERSE_POLICY,
  ...
  const recentMedian = median(quoteVolumes.slice(-30 * 24 * 4));
  ...
  if (historicalCoverageDays < policy.minimumHistoryDays) reasons.push("HISTORY");
