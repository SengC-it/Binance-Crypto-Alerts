...
const REQUIRED_FREEZE_START = BASELINE;
...
async function runFreeze(): Promise<void> {
  if (currentHead() !== REQUIRED_FREEZE_START) throw new Error(`V15 freeze must start at the exact baseline ${REQUIRED_FREEZE_START}; current ${currentHead()}`);
