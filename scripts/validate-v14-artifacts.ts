...
const required = [
  "v14-correctness-freeze-manifest.json",
  "v14-data-gate.json",
...
const freeze = documents.get("v14-correctness-freeze-manifest.json") as Record<string, unknown>;
...
for (const key of ["requestedArchiveSlots", "availableZipCount", "checksumPassZipCount", "missingSlotCount", "failedZipCount"]) assertCondition(Number.isInteger(evidence[key]) && Number(evidence[key]) >= 0, `invalid evidence count: ${key}`);
...
assertCondition(["V14_CROSS_SECTIONAL_REVERSAL_REJECTED", "V14_CORRECTNESS_PASS_EXTERNAL_CONFIRMATION_REQUIRED"].includes(String(summary.result)), "invalid V14 result state");
