...
async function main(): Promise<void> {
  ...
  requireThat((await git(["rev-parse", `${targetHead}^`])) === WP1_SHA, "WP2 must be exactly one commit after WP1.1");
  ...
  requireThat(manifest.schema === "v22-wp2-freeze-manifest-v1" && manifest.experimentId === V22_EXPERIMENT_ID && manifest.branch === V22_BRANCH && manifest.baseSha === V22_BASE_SHA && manifest.directParent === WP1_SHA && manifest.wp1_1AcceptedCommit === WP1_SHA && manifest.wp1_1ManifestSha256 === WP1_MANIFEST_SHA, "freeze manifest identity drifted");
  ...
  const changed = (await git(["diff", "--name-only", `${WP1_SHA}..${targetHead}`])).split(/\r?\n/).filter(Boolean);
