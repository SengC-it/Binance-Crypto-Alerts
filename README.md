...
- Supabase 只保存 `cs_` 前缀的最新结果、信号事件、扫描状态和优化结果；原始历史行情放在本地 `data/raw/`，不会提交到公开仓库。
...
迁移文件：[`supabase/migrations/20260808235907_cs_initial_schema.sql`](./supabase/migrations/20260808235907_cs_initial_schema.sql) 和 [`supabase/migrations/20260809000642_cs_claim_signal_fix.sql`](./supabase/migrations/20260809000642_cs_claim_signal_fix.sql)。已应用到项目 `jfvbikivtpfjgfsnggiz`，只新增以下 `cs_` 对象：

- `cs_instruments`、`cs_scan_runs`、`cs_signals`、`cs_signal_events`
- `cs_risk_budgets`、`cs_notifications`、`cs_system_events`
- `cs_strategy_versions`、`cs_backtest_runs`、`cs_app_settings`
- `cs_claim_signal`、`cs_set_updated_at` 函数及对应索引/触发器
...
2. 在 Supabase Vault 中保存 `cs_scan_url` 和 `cs_cron_secret`。
3. 按 [`supabase/scheduler.sql`](./supabase/scheduler.sql) 创建 `cs-scan-batch-0..3`。这不会改动数据库中已有的 Cron。
...
先设置 `CS_HISTORY_SYMBOLS`，再运行 `pnpm history:download` 从 Binance 公共接口下载本地历史 K 线和资金费率；建议先从 BTC/ETH 等少量代表币种开始，确认存储空间和运行时间后再扩大到更多币种。`pnpm optimizer` 从 `data/raw/*.json` 读取本地历史数据，生成 54 组参数变体，按 9 个月训练 + 3 个月样本外评估，并以最大回撤 30% 作为资格线。合格的版本写入 `cs_strategy_versions`，运行摘要写入 `cs_backtest_runs`。
